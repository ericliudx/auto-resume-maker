...
export function ResumePreview({
  ...
  const [fitInfo, setFitInfo] = useState<{
    cfg: { expLimit: number; projLimit: number; expBullets: number; projBullets: number }
    fittedBank: BioBank
  } | null>(null)
  ...
  const trimSummary = useMemo(() => {
    ...
    const expDropped = Math.max(0, effectiveBank.experiences.length - fittedBank.experiences.length)
    const projDropped = Math.max(0, effectiveBank.projects.length - fittedBank.projects.length)
    const expBulletsDropped =
      effectiveBank.experiences.reduce((n, e) => n + (e.bullets?.length ?? 0), 0) -
      ...
      >
        ...
        {trimSummary && fitEnabled ? (
          <div className="mt-2 mx-2 rounded-xl border border-[var(--border)] bg-[var(--bg)] p-2 text-xs text-[var(--text)]">
            Fit summary: expLimit {trimSummary.cfg.expLimit}, projLimit {trimSummary.cfg.projLimit}, expBullets{' '}
            {trimSummary.cfg.expBullets}, projBullets {trimSummary.cfg.projBullets}. Trimmed: {trimSummary.expDropped}{' '}
            exp, {trimSummary.projDropped} proj, {trimSummary.expBulletsDropped} exp bullets,{' '}
