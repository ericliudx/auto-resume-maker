...
export function buildTailorPrompt(args: {
  ...
  return [
    ...
    '- You MUST include top-level "pdfFileName": one string First_Last_Company (first name, last name, short employer token from the job posting). ASCII letters and digits only, underscores between segments, at least three segments (example: Eric_Liu_Affirm). The app saves this and uses it as the browser print-to-PDF default filename; omitting or invalid values will fail validation.',
    "",
    "Task:",
    ...
    "",
    "Output JSON shape (pdfFileName is required):",
    "{",
    '  "experienceIds": ["<id>", "..."],',
    '  "projectIds": ["<id>", "..."],',
    '  "pdfFileName": "Eric_Liu_CompanyToken",',
